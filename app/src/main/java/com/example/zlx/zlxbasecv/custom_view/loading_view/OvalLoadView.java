package com.example.zlx.zlxbasecv.custom_view.loading_view;
...
public class OvalLoadView extends View {
    ...
    public OvalLoadView(Context context) {
        this(context, null);
    }

    public OvalLoadView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OvalLoadView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        //获取自定义属性
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.OvalLoadView);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.OvalLoadView_progress_color) {
                mProgressColor = typedArray.getColor(attr, PROGRESS_COLOR);

            } else if (attr == R.styleable.OvalLoadView_progress_width) {
                mProgressWidth = (int) typedArray.getDimension(attr, mProgressWidth);

            } else if (attr == R.styleable.OvalLoadView_radius) {
